...
def evaluate_answer(test: TestQuestion) -> tuple[AnswerEval, str, list]:
    ...
    judge_prompt = f"""You are an expert evaluator assessing the quality of answers from an
internal IT help-desk assistant tool. The tool's audience is a technician, not the end
customer, so don't penalize answers for being written to the technician rather than the
customer. Evaluate the generated answer by comparing it to the reference answer.
...
Provide detailed feedback and scores from 1 (very poor) to 5 (ideal) for each dimension. If the answer is wrong, then the accuracy score must be 1."""
    ...
    answer_eval = structured_judge.invoke(judge_prompt)
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: uv run python -m evaluation.eval <test_row_number>")
        sys.exit(1)
